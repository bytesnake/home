...
\begin{definition}[label=p1f9h5bp, name=Structured Matrices]
	A structured matrix represents a linear operation \( T: \mathbb R^n \to \mathbb R^m\)
	without the required operation and parameter count of \( mn \) for
	storage and matrix-vector multiplication.

	\begin{example}[label=9v_h8bhy, name=Low Rank]
	\end{example}
\end{definition}
